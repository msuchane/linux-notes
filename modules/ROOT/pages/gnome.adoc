...
In earlier versions of GNOME, you could enable volume over-amplification in GNOME Tweaks. The setting is missing now in Tweaks and in Settings.

You can enable over-amplification using `gsettings`:
...
By default, you can only copy and paste one item at a time. With the Clipboard History extension to GNOME Shell, you can switch to previously copied items.

Enable the extension: link:https://extensions.gnome.org/extension/4839/clipboard-history/[Clipboard History by SUPERCILEX].
