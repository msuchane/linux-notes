...
The BFQ disk scheduler provides better interactive performance even on SSD and NVMe disks. You can enable BFQ on all storage devices:

. Load the BFQ module.
+
Create the `/etc/modules-load.d/bfq.conf` file and enter `bfq`.

. Set the BFQ scheduler for all devices.
+
Create the `/etc/udev/rules.d/60-scheduler.rules` file with the following configuration:
+
----
...
. Check that the BFQ scheduler is active on all devices:
+
[source,bash]
----
