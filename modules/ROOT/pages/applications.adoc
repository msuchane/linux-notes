= Configuring applications [[applications]]
...
=== Scale web pages and the interface [[web-scaling]]
...
* Increase the minimal font size in Firefox settings

* Adjust the CSS pixels scaling ratio
+
The benefit of this approach is that the scaling is:
...
To enable CCS pixels scaling, go to the `about:config` page and set the following key to a comfortable value, e.g. `1.5`:

* `layout.css.devPixelsPerPx`

=== Disable pasting with the middle mouse button [[middle-mouse-paste]]
...
=== Disable the Alt main menu [[alt-menu]]
...
=== Configure the session backup interval [[session-interval]]
...
=== Prefer the native PDF reader [[firefox-pdf]]
...
=== Improve font aliasing [[gimp-font-alias]]
...
. In `menulibre`, navigate to menu:Internet[Chromium Web Browser].
...
. Press kbd:[Ctrl + S] to save the launcher.
